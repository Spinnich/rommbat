...
**Two of seven rows certified at `5.3.1`, on 2026-09-26: `bizhawk`/`Nymashock` and `Octoshock`**,
all nine steps with step 6 N/A, since BizHawk exposes no memory card option. The other five pass
every step but 6, which drives the memory card modes they expose and is owed; see the end.
...
**Step 6 on the five rows with card options, and nothing else.** Steps 1 to 5 and 7 to 9 pass on all
seven. DuckStation's `Shared`, `PerGameFileTitle` and `PerGame`; `swanstation`'s `Shared`,
`PerGame` and `PerGameTitle`; `mednafen_psx_hw`'s shared card; `pcsx_rearmed`'s second card; and
mednafen's card counts `1`, `3` and `4`. A shared card needs two games saving to it to show whether
RomMBat keeps it off both. The two BizHawk rows expose no card option, so step 6 is N/A there, and
they are the only rows that could be called certified today.
