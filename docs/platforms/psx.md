...
**Certified at `5.3.1` on 2026-09-26: all seven rows**, every step, with step 6 N/A on the two BizHawk rows, which expose no memory card option. Each non-`libretro` row, and each non-default card type, needed code first; findings 328 to 335 are the record of what it took.
...
| 3   | **Pass, needs the file.** Without it the core stops on "Firmware is missing: scph5501.bin", its own region default, since RetroBat points it at `psxonpsp660.bin` and that was absent                                                                                                                                                                                                                                                                 |
...
| 6   | **Pass.** `beetle_psx_hw_enable_memcard1` on adds a loose `<rom>.1.mcr` for port 2 beside the `.srm`; SotN loaded and saved to it, it went up in `libretro:battery:mcr`, and deleted and restored it came back byte for byte, `f2e41388...`. `beetle_psx_hw_shared_memory_cards` on wrote only the per-game `.srm`, since at RetroBat's defaults the left card keeps RetroArch's method and the right card is off, so it shares nothing (finding 335) |
...
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 2   | Carried from the multi-disc pass above                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| 3   | **Pass, needs the file.** Without it RetroArch exits 1: "Failed to load NTSC-U BIOS. System failed to boot"                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| 6   | **Pass, every card type.** `PerGameTitle` on port 1 and `PerGame` on port 2 wrote loose `Castlevania - Symphony of the Night (USA)_1.mcd` and `SLUS-00067_2.mcd`, each saved to, sent in `libretro:battery:mcd` and `:mcd2`, and restored byte for byte under its own name after two fixes (finding 335). `Shared` wrote `duckstation_shared_card_1.mcd`, seeded with both games' saves; SotN and Metal Gear Solid each saved to it, and it is reported as shared by several games and never sent. `None` writes no card and is not driven |
...
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2   | Carried from the multi-disc pass above                                                                                                                                                                                                               |
| 3   | **Pass, boots without it.** SotN reached its title screen on the core's own HLE BIOS                                                                                                                                                                 |
...
| 6   | **Pass.** `pcsx_rearmed_memcard2` on adds `pcsx-card2.mcd`, one card every game writes to; SotN and Metal Gear Solid each saved to it, and it is reported as shared by several games and never sent. Port 1 keeps the per-game `.srm`                |
| 7   | Carried: the same entry and media as the stock row                                                                                                                                                                                                   |
| 8   | **Pass.** `status` reads this row's ES session back against its rom; see the table below                                                                                                                                                             |
| 9   | **Pass.** After the restore, `flush` uploaded nothing and the sync preview answered nothing to do                                                                                                                                                    |
...
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2   | Carried from the multi-disc pass above                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| 3   | **Pass, needs the file.** Without it `-batch` exits at once; run windowed it reports "Failed to open BIOS 'psxonpsp660.bin'"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| 6   | **Pass, every card type**, driven 2026-09-26 with the hooks uninstalled so nothing flushed before the scan was read. `Shared`: SotN and Metal Gear Solid both saved to one `shared_card_1.mcd`, seeded with both saves, and DuckStation added `shared_card_2.mcd`; the scan reports both as shared by several games, records no save and binds no game, and the flush sent nothing. `PerGame`: SotN saved to `SLUS-00067_1.mcd`, attributed through its launch and sent, with the title card beside it reported as superseded (finding 333). `PerGameFileTitle`: Metal Gear Solid (USA) (Rev 1) saved to `Metal Gear Solid (USA) (Rev 1)_1.mcd`, the playlist's stem, attributed to rom 320306 and sent; deleted and restored it came back byte for byte, `bc90ca61...`. `None` and `NonPersistent` write no card and are not driven                                                                                                                                                                                                                                                                                                                          |
| 7   | Carried: the same entry and media as the stock row                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| 8   | **Pass.** `status` reads this row's ES session back against its rom; see the table below                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| 9   | **Pass.** After the restore, `flush` uploaded nothing and the sync preview answered nothing to do                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2   | Carried from the multi-disc pass above                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| 3   | **Pass, needs the file.** Without it mednafen reports an error opening `R:\RetroBatios\scph5501.bin`, its NTSC-U default, since `psx.bios_na` names `psxonpsp660.bin`                                                                                                                                                                                                                                                                                                                                                                                            |
...
| 6   | **Pass.** At `2` cards the row passed steps 4 and 5 above. Booted through `emulatorLauncher` at `1` and `4`, mednafen wrote only `.0.mcr` at `1`, and `.0.mcr` and `.1.mcr` at `4` although `mednafen.cfg` enabled ports 1 to 4, since ports 3 and 4 need a multitap; those are the files the rule proven at `2` carries, measured by boot rather than by a save. `0` is RetroBat's default and has no card (finding 330). mednafen also keeps rolling copies of a card under `mednafen/backup/`, which are not saves and are reported as a shape nothing covers |
...
**Nothing for the certification.** Two things are known and recorded rather than owed: a restore fills only a slot no file holds, so a live card deleted beside a stale one of the same game restores nothing (finding 333); and a restored DuckStation state loads only from ES's save-state menu.
