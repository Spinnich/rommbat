...
| 6   | N/A         | Unchanged: `mastersystem` has no class D                                                                          |
...
the deploy, with `mastersystem` resolved by `fs_slug` as before. `sync` answered `nothing to do: 153
games already present, 0 downloaded, 0 written` for `Smart collection 8`, and `gamelists: all 8 unchanged`,
