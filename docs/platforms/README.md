...
zipped ROM only through a bare `.gba` beside it, which RetroBat deletes on exit, and keeps its saves
outside `saves/` (finding 286). Three rows refuse to boot without `gba_bios.bin` and seven do not
