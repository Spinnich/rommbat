...
Read in RomM at tags `5.2.0`, `5.3.0-alpha.2` and `5.3.0-alpha.3`, and checked against the restore preview on the `nes` install. The link rule is identical in all three, and in `5.3.0-beta.1` and `5.3.0`, the floor now: neither `db_screenshot_handler` nor `compute_file_name_no_ext` changes between `alpha.3` and `5.3.0`.
