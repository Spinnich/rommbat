...
2. **The per-game VMU is named after the disc's product number, not the rom file.**
   ...
   launch window from `emulatorLauncher.log`.
...
`PLAN.md` lines 148-150, 375 and 750 all cite `%~dp0..\..\..\` as the way a hook resolves the
agent. The _pattern_ is right and the technique is confirmed working, but a hook invoking an
agent at `<root>/plugins/rommbat/` needs `%~dp0..\..\..\..\plugins\rommbat\`.
...
| 2   | Hooks resolve the agent through `%~dp0..\..\..\` (L148-150, L375, L750)                               | Three levels reaches `emulationstation/`; the root needs four                                                                                                                                                                                                                           |
...
| 9b  | DuckStation `PerGameTitle` is treated as sufficient (L831)                                            | `PerGameFileTitle` is the better value: it keys the card by **rom filename** rather than by DuckStation's internal title                                                                                                                                                                |
...
| 49  | Dreamcast converts to per-game via `flycast_vmupergame` (finding 9 above)                             | Confirmed live, but the per-game VMU is named for the **disc serial** (`T40217N_vmu_save_A1.bin`), not the rom file, so it does not collapse into class A the way DuckStation's `PerGameFileTitle` does                                                                                 |
