...
**What is still only reasoned.** `KeepLocalAsync` records a sync record for the row it wrote
and never acks the row it superseded, so that row keeps a permanent gap in this device's sync
history. 163 makes the gap unreachable rather than filling it, which is sound while negotiate
pairs on the newest row per slot and is a dependency on server behaviour rather than on
anything this client controls.
