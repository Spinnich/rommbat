...
| 283 | Where Kega Fusion writes a `megadrive` battery save                           | **`emulators/kega-fusion/<rom>.srm`, outside `saves/`.** RetroBat's template `Fusion.ini` sets `SRMFiles=.\..\..\emulators\kega-fusion` beside `StateFiles=.\..\..\saves\megadrive\kega-fusion`, and `emulatorLauncher` rewrote neither over five launches. A RetroBat defect to report upstream: RomMBat reads saves from `saves/` only, and writing the key is ruled out by core principle 2. States land in `saves/megadrive/kega-fusion/<rom>.gs<slot>`, `F5` saving and `F7` stepping the slot down with a wrap from 0 to 9. Kega is not bundled: the folder held only the template until ES downloaded the emulator on a first launch. `auto`, `genesis` and `megadrive` are its `-auto`, `-gen` and `-md` flags                                                                                                                                                     |
| 284 | Whether Kega Fusion is playable by pad from ES                                | **Not on this install.** `emulatorLauncher` logged `No specific mapping found for megadrive controller` and wrote `Joystick1Using=3` for player 1 with the Xbox 360 pad on device 0, and RetroBat ships no pad-to-key file for `kega-fusion`, so no pad button saves a state. The keyboard reaches the game (numpad 8, 2, 4, 6; `J`, `K`, `L`; Right Ctrl for Start), but Sonic 3 & Knuckles went from the title straight into the game under `-auto` and `-gen`, skipping data select, so no new battery save could be made. Why it skips is not determined                                                                                                                                                                                                                                                                                                               |
