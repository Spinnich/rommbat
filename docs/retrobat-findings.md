...
| 286 | Whether `nosgba` boots a zipped `gba` library                | **No.** `emulatorLauncher` runs `no$gba.exe /f "<rom>.zip"` and NO$GBA answers "Cartridge not found". The same ROM extracted to a bare `.gba` outside the tree, launched the same way, boots without the BIOS, though its INI sets "Emulate BIOS Functions" to "By real GBA.ROM". So the row boots none of a set RomM serves zipped. NO$GBA has no save-path key in its INI and a `BATTERY`folder in`emulators/nosgba/`, so its saves likely land outside `saves/` as Kega Fusion's do (**283**); unmeasured, since no real save could be made on a library it cannot open                                                                                                                                                                                                                                                     |
