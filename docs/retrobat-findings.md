...
| 179 | ES writes `es_settings.cfg` on exit, only when a setting changed that session (**M0 probe 2, finding 33**) | **It also writes at startup, and it writes the moment a setting changes rather than waiting for exit.** On two independent installs ES added `Language` at launch (`E:` at 06:32, `K:` at 06:38), merging into a file it had just read. Then toggling one option in the ES UI rewrote the file immediately, at 06:41, with ES still running and the quit yet to come. So the window in which RomMBat's write is safe is not "between ES's exit and its next start" but strictly "while ES is not running", and the count of writes per session is not one |
