...
saves, each seeded with the save the one before made. `docs/platforms/gb.md` has the passes.
...
| 295 | What the `libretro` `.rtc` on `gb` holds                            | **Only the host time at exit, for a cartridge with no clock.** `tgbdual`, `DoubleCherryGB` and `sameboy` write `<rom>.rtc` as RetroArch's RAM type 1 beside the `.srm` for every game. `tgbdual` and `DoubleCherryGB` write 4 B, a little-endian Unix time equal to the session's end; `sameboy` writes 32 B, zeroed clock registers with the same kind of timestamp at offset 16, and read `DoubleCherryGB`'s 4 B file without complaint, keeping those bytes at offset 0. No cartridge in the 107-game set has a clock (header type `0x0F` or `0x10`), and RomM files the clock-cart Pokemon titles under `gbc`. By the maintainer's ruling `.rtc` is not a save on `gb`, through `save_rules.json`'s `not_a_save_by_system`; `gbc` decides for itself                                                                                                                                                                                                                                                                                                                                                                                       |
