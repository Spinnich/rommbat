...
Already pinned. `src/RomM.Client/openapi/romm-5.3.0-alpha.3.json` is a byte-exact
`/openapi.json` (served at the root, not under `/api`) from a server reporting
**5.3.0-alpha.3**, the minimum RomMBat supports, so the generated DTOs describe the oldest
