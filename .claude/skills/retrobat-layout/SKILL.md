...
generated by `tools/build-bios-manifest.py`. An entry carrying `supplement` is one RetroBat lists
under a sibling system, copied onto a system whose row was measured reading it (`gb` takes
`sgb`'s four and `gbc`'s boot ROM, finding 293).
