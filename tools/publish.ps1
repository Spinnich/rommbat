...
# both. Naming files explicitly is what keeps the .pdb files out: a publish emits 105 MB of
# them beside a 194 MB payload, and a wildcard copy ships the lot.
$manifest = @(
...
$layout = Join-Path $OutputPath 'rommbat'
...
if (-not $NoZip) {
    ...
    Compress-Archive -Path (Join-Path $layout '*') -DestinationPath $zip
...
# The same markers RootMarkers.IsRoot uses, for the same reason: pointing at the wrong
# directory and quietly building a tree there is worse than being told the path is not a
# RetroBat install.
